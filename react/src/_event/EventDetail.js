...
const EventDetail = () => {
	...
	const thumbnail = event?.attributes?.banner?.data?.attributes?.formats?.thumbnail?.url;
	...
	return (
		<Box>
			...
			/>
			<Container bgcolor="background.default" sx={{ padding: '40px 20px' }}>
				<Typography variant="h2">{event?.attributes[`name_${i18n.language}`]}</Typography>
				...
				<Box mt={10} sx={markdownStyle}>
					...
					>
						<img
							...
							alt={event?.attributes.banner?.data?.attributes?.alternativeText}
						/>
						<img
							loading="lazy"
					...
					<Markdown className="markdown">{event?.attributes?.content}</Markdown>
