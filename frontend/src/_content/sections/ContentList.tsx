...
const Feed = () => {
	...
	const [page, result] = useMemo(() => {
		return [~~query.get('page') || 1, query.get('result') || '']
	}, [query])

	const fetchPosts = () => {
		const query =
			result &&
			result.split(',').reduce((str, el, index) => {
				return str + `&filters[id][$in][${index}]=${el}`
			}, '')
		strapiAxios
			.get(
				`/api/publications?populate=banner&pagination[page]=${page}&pagination[pageSize]=6${query}`
			)
			.then(({ data }) => {
				setPosts(data.data)
