...
const ContentSidebar = ({ post }) => {
	...
	const authors =
		post?.attributes?.authors?.data.map((author) => ({
			id: author.id,
			name:
				author.attributes[`prefix_${i18n.language}`] +
				' ' +
				author.attributes[`name_${i18n.language}`] +
				' ' +
				author.attributes[`surname_${i18n.language}`],
	...
	return (
		<Stack
			sx={{ padding: '24px 20px' }}
			width={{ xs: 'calc(100% - 40px)', md: 300 }}
		>
			{/* Author */}
